...
class SchnorrZKP:
    ...
    def prove(secret: bytes, public_data: Dict, additional_info: bytes = b"") -> Dict:
        """
        生成“可验证证明”：
        - 使用 PQC 签名（liboqs）对 payload 做签名
        - payload 内包含 secret（用于与 k_session 绑定）与 public_data
        ...
        """
        if not HAS_OQS:
            raise RuntimeError("liboqs-python 未安装：无法生成 PQC 证明")

        # 1) 构造待签名上下文，payload 里显式绑定 secret
        ...
        # 2) 为该证明生成临时 PQC keypair（proof 内携带 public_key）
    ...
    def verify(
        ...
        expected_secret: Optional[bytes] = None,
    ) -> bool:
        """
        验证 PQC 证明（对称于上面的 prove）。

        expected_secret 用于重建 payload，从而确保“与 k_session 绑定”可被服务端验证。
        ...
        try:
            ...
            sig = oqs.Signature("ML-DSA-65")
            return bool(sig.verify(payload, signature_bytes, public_key_bytes))
    ...
    def verify(
        ...
        try:
            ...
            if not proof_nonce:
                ...
                return False
            # ========== 检查 1.1：nonce 重放（支持 Redis/多实例） ==========
            if replay_backend is not None:
                # tag 即 linkage_tag（字符串），用于构造 nonce 的唯一键
                if not replay_backend.check_nonce(proof_nonce, public_inputs["tag"]):
                    logger.warning("[Verify] 重放攻击：nonce 已使用（持久化后端）")
                    return False
            else:
                # 退化到进程内集合（开发/单实例）
                if proof_nonce in cls.seen_nonces:
                    logger.warning("[Verify] 重放攻击：nonce 已使用（进程内）")
                    return False
            ...
            is_schnorr_valid = SchnorrZKP.verify(
                ...
                expected_secret=binding_secret,
            )
...
class TrueLinkableRingSignature:
    ...
    def __init__(self, replay_backend=None):
    ...
    def sign(self, message: bytes, epoch: int, k_session: bytes, id_v: str,
        ...
        leaf_pk = path_info.get("leaf_pk")
        if leaf_pk is None:
            raise ValueError("[Sign] prover_xmss 未提供 leaf_pk，无法构造环签名")
        ...
        # 不覆盖调用方传入的 ring_path/ring_index：
        # 真实的环成员证明应该以调用方提供的 (ring_root, ring_path, ring_index) 为准。
        if ring_path is None or ring_index is None:
            raise ValueError("[Sign] ring_path/ring_index 缺失")
        ...
        # ========== 生成 ZKP（包含完整验证逻辑） ==========
        k_session_binding = hashlib.sha256(k_session).digest()
        k_session_hash = k_session_binding.hex()
        ...
        return {
            ...
            "leaf_pk": leaf_pk.hex(),
            "zk_proof": zk_proof,
            # 让服务端可在未提供 k_session 明文时仍能重建 binding_secret
            "k_session_hash": k_session_hash,
            "timestamp": int(time.time())
        }
    ...
    def verify(
        ...
        message: bytes,
        signature: Dict,
        ring_root: bytes = None,
        k_session: bytes = None,
    ) -> bool:
        """
        验证环签名（包含完整的密码学检查）
        ...
        required_fields = [
            ...
            "leaf_pk", "zk_proof", "timestamp", "k_session_hash"
        ]
        ...
        try:
            ...
            # 绑定 secret = SHA256(k_session)，用于重建/验证 proof payload
            binding_secret = bytes.fromhex(signature["k_session_hash"])
            if k_session is not None:
                expected = hashlib.sha256(k_session).digest().hex()
                if signature["k_session_hash"] != expected:
                    logger.warning("[Verify] k_session_hash 与当前 k_session 不一致")
                    return False
